...
export default function MissingPage() {
  ...
  function buildMissingMessage(): string {
    const lines = [
      ...
      `👉 ${APP_URL}`,
      '',
      'Mis figuritas *faltantes* del Mundial 2026:',
      '',
    ]
    for (const group of GROUPS) {
      ...
      for (const team of group.teams) {
        ...
        lines.push(`${team.flag} ${team.name}: ${tm.map(s => s.id).join(', ')}`)
      }
      lines.push('')
    }
    const fwcM = missing.filter(s => s.type === 'intro' || s.type === 'museum')
    if (fwcM.length > 0) lines.push(`🏅 FWC: ${fwcM.map(s => s.id).join(', ')}`)
    const ccM = missing.filter(s => s.type === 'cocacola')
    if (ccM.length > 0) lines.push(`🥤 Coca-Cola: ${ccM.map(s => s.id).join(', ')}`)
    return lines.join('\n')
  }
